...
export function ProjectDashboard({
  ...
  const [internalMessagesOpen, setInternalMessagesOpen] = useState<boolean>(
    lsKey
      ? readLS(lsKey("messagesOpen"), defaultMessagesOpen)
      : defaultMessagesOpen
  );
  ...
  >(
    lsKey
      ? readLS(lsKey("statusFilter"), defaultStatusFilter)
      : defaultStatusFilter
  );
  ...
  const dataProjects =
    onProjectUpdate || onProjectsReorder ? projects : localProjects;
  ...
  const preparedProjects = useMemo(() => {
    ...
    if (activeStatusFilter !== "all") {
      list = list.filter(
        (p) => (p.status ?? "inProgress") === activeStatusFilter
      );
    }
    if (q) {
      list = list.filter(
  ...
  // Pagination
  const totalPages = virtualizeList
    ? 1
    : Math.max(1, Math.ceil(preparedProjects.length / pageSize));
  ...
  const endIndex =
    virtualizeList && viewMode === "list"
      ? Math.min(
          totalCount,
          Math.ceil((scrollTop + viewportH) / itemH) + overscan
        )
      : totalCount;
  ...
  // Persistence
  useEffect(() => {
    if (lsKey) writeLS(lsKey("view"), viewMode);
  }, [lsKey, viewMode]);
  useEffect(() => {
    if (lsKey) writeLS(lsKey("query"), query);
  }, [lsKey, query]);
  useEffect(() => {
    if (lsKey) writeLS(lsKey("messagesOpen"), isMessagesOpen);
  }, [lsKey, isMessagesOpen]);
  useEffect(() => {
    if (lsKey) {
      writeLS(lsKey("sortBy"), activeSortBy);
      writeLS(lsKey("sortDir"), activeSortDir);
    }
  }, [lsKey, activeSortBy, activeSortDir]);
  useEffect(() => {
    if (lsKey) writeLS(lsKey("statusFilter"), activeStatusFilter);
  }, [lsKey, activeStatusFilter]);
  useEffect(() => {
    if (lsKey && !virtualizeList) writeLS(lsKey("page"), currentPage);
  }, [lsKey, currentPage, virtualizeList]);

  // Keyboard
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (isMessagesOpen) updateMessagesOpen(false);
  ...
  const updateMessagesOpen = (open: boolean) => {
  ...
  const saveEdit = () => {
    ...
    if (onProjectUpdate) {
      ...
      setLocalProjects((arr) =>
        arr.map((x) => (x.id === editDraft.id ? editDraft : x))
      );
    }
    setEditingId(null);
    setEditDraft(null);
  ...
  const moveBy = (id: string, delta: number) => {
    ...
    const reordered = reorderWithinFull(
      dataProjects.map((p) => p.id),
      vis
    );
    doReorder(reordered);
  ...
  return (
    <div
      className={cx(
        "pd-root flex flex-col h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100",
        className
      )}
    >
      ...
      >
        ...
        <div className={cx("flex items-center", spacing.gap.xs)}>
          ...
          <button
            ...
            onClick={() => updateMessagesOpen(true)}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div
              className={cx("flex flex-wrap items-center", spacing.gap.md)}
            >
              {computedStats.map((s, i) => (
                <div
                  key={s.id}
                  className={cx("flex items-center", spacing.gap.xs)}
                >
            ...
            <div className={cx("flex items-center", spacing.gap.xs)}>
              ...
              >
                ...
                <select
                  ...
                  value={activeSortBy}
                  onChange={(e) =>
                    setSort(e.target.value as SortBy, activeSortDir)
                  }
                  className={cx(
                    "rounded-lg ring-1 ring-slate-200 dark:ring-slate-700",
                    "bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200",
              ...
              <button
                className={cx(
                  "p-2 rounded-lg ring-1 ring-slate-200 dark:ring-slate-700 transition-colors text-sm",
                  reorderMode
                ...
                <div className="animate-pulse grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {Array.from({ length: Math.min(pageSize, 6) }).map(
                    (_, i) => (
                      <div
                        key={i}
                        className="h-44 rounded-xl bg-slate-200 dark:bg-slate-700"
                      />
                    )
                  )}
                </div>
              ...
              visibleProjects.map((p) => {
                ...
                return (
                  <article
                    ...
                    onDragStart={() =>
                      canReorder && handleDragStart(p.id)
                    }
                    onDragOver={
                      canReorder ? handleDragOver : undefined
                    }
                      ...
                      >
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={cancelEdit}
                            className={cx(
                              "rounded-lg ring-1 ring-slate-300 dark:ring-slate-700 text-slate-700 dark:text-slate-200",
                              spacing.button.sm
                            )}
                          >
                            Отказ
          ...
          {!loading &&
            !virtualizeList &&
            preparedProjects.length > pageSize && (
              <div
                className={cx(
        ...
        >
          ...
          >
            ...
            <button
              ...
              onClick={() => updateMessagesOpen(false)}
                ...
                <div className="h-2 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor:
                        detailProject.accentColor || "#6366f1",
                    }}
                  />
                </div>
              </div>
              {detailProject.participants &&
